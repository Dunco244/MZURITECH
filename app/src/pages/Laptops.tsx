...
// Filter static products for laptops category
const laptopStaticProducts = staticProducts.filter(p => 
  p.category === 'laptops' || p.category === 'gaming'
);
...
export default function Laptops() {
  ...
  const [products, setProducts] = useState<Product[]>(laptopStaticProducts);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_URL}/api/products?category=laptops&limit=50`);
        if (response.ok) {
    ...
    <div className="min-h-screen bg-gray-50 pt-28 sm:pt-24 pb-16">
      <div className="section-padding">
        <div className="container-custom">
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-secondary">Laptops & Gaming</h1>
            <p className="text-gray-600 mt-2">Premium laptops for work, creativity, and gaming</p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-16 bg-white rounded-2xl">
              <p className="text-gray-500">No laptops available at the moment.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
