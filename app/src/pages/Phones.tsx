...
// Filter static products for phones category
const phoneStaticProducts = staticProducts.filter(p => p.category === 'phones');
...
export default function Phones() {
  ...
  const [products, setProducts] = useState<Product[]>(phoneStaticProducts);
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const response = await fetch(`${API_URL}/api/products?category=phones&limit=50`);
        if (response.ok) {
    ...
    <div className="min-h-screen bg-gray-50 pt-28 sm:pt-24 pb-16">
      <div className="section-padding">
        <div className="container-custom">
          {/* Header */}
          <div className="mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-secondary">Phones</h1>
            <p className="text-gray-600 mt-2">Browse our latest smartphones</p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="col-span-full text-center text-gray-500 py-12">No phones found.</div>
          )}
        </div>
      </div>
